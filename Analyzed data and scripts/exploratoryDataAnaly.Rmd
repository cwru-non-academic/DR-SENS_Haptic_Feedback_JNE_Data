...
electrodeData.sum %>%
    ggplot(data = electrodeData, mapping = aes(x=reorder(electrode, -percentage), y=percentage)) +
    geom_bar(data = electrodeData.sum, stat='identity', fill = "#abd5f5", color ="#349beb")+
    #geom_errorbar(data = electrodeData.sum, mapping =aes(ymin=mean-sd, ymax=mean+sd), width =0.3)+
    geom_point(color="#349beb", alpha = 3/5, size = 4)+
...
combinationData.sum %>%
    ggplot(data = combinationData, mapping = aes(x=reorder(combination, -percentage), y=percentage)) +
    geom_bar(data=combinationData.sum,stat='identity', fill = "#abd5f5", color ="#349beb")+
  ...
  theme(legend.position = "bottom", axis.text=element_text(size=12), axis.title=element_text(size=13), legend.text=element_text(size=12),legend.title =element_text(size=13), plot.title=element_text(size=14))
ggsave("explo_res_combinationR1.jpg")
combinationData.sum 
```
...
Percentage per pulse type count
```{r}
polarityData.pulse <- polarityData %>%
  group_by(pulse) %>%
  summarise( 
    n=n(),
    use=sum(useful_for_polarity),
    mean=mean(percentage),
    sd=sd(percentage),
    combination=combination,
    pulse=pulse
...
```{r}
polarityData.sum %>%
    ggplot(data = polarityData, mapping = aes(x=combination, y=percentage, fill = factor(pulse, levels =c('Anodic', 'Cathodic', 'Neither(Same area)', 'Useful')), color = factor(pulse, levels =c('Anodic', 'Cathodic', 'Neither(Same area)', 'Useful')))) +
    geom_bar(data = polarityData.sum, stat='identity',position = position_dodge(width=0.9), alpha = 1/8)+
    geom_point(alpha = 3/5, size = 3, position = position_dodge(width=0.9))+
  ...
  scale_color_manual(name="",
        ...
        labels=c("PD Cathodic Leading on\nMore Distal Electrode", "PD Cathodic Leading on\nMore Proximal Electrode" , "PD Electrodes on Same\nPhalanx or Palm Region", "PI Useful\nTrials"))+
...
ggsave("explo_res_polarityCombR1.jpg")
