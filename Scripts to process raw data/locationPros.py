# -*- coding: utf-8 -*-
"""
Created on Fri Nov  5 11:23:25 2021
...
def rgb(value):
    ...
    maximum = 8
    if value>maximum:
        print("value overload"+str(value))
    value=min(value, maximum)
    ratio = 1 * (value-minimum) / (maximum - minimum)
    [r, g, b] = interpolate(turbo_colormap_data, ratio)
    ...
    while i < colorArray.shape[0]:
        while j < colorArray.shape[1]:
            if colorArray[i,j] > 0:
                ...
                if 500 < i <1000:
                        if 250 < j < 1000:
                            #intensity = round((250.0/totalImages)*colorArray[i][j])
                            imageC[i][j] = rgb(colorArray[i][j])
            j=j+1
        i=i+1
        j=0
 
    plt.imshow(imageC)
    plt.savefig(outputFileName+location+strenght+".png", dpi = 2000)
    #plt.savefig("results"+location+strenght+".pdf", dpi = 2000)
    
#for loc in range(len(locationList)):
    #for stren in range(len(strenghtList)):
        #batch(locationList[loc], strenghtList[stren])
        
outputFileName = "tResultsMaxSens"
